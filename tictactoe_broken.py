...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player #+1 removed
    return game
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        while not available:  #colon added
            ...
            available = check_space_empty(game, row, column) #column added as argument
        game = add_piece(game, player, row, column)
        display_game(game)
        player = switch_player(player)
        winner = check_winner(game) #function call was commented
